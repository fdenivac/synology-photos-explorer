...
from PyQt6.QtWidgets import QAbstractButton, QGraphicsColorizeEffect, QWidget, QPushButton
...
class SvgAbstractButton(QAbstractButton):
    ...
    def __styleInit(self):
        # avoid log message "QFSFileEngine::open: No file name specified" due to property "image:"
        propUrl = f"image: url({self.__icon});" if self.__icon else ""
        self.__btn_style = f"""
        QAbstractButton
        {{
        border: 0;
